...
DEFAULT_OUTPUT_ASSIGNMENT = r"projects/ZOOKEEPER/ghs_assignment.csv"
DEFAULT_OUTPUT_SCORE = r"projects/ZOOKEEPER/ghs_score.json"
    ...
    def run(self):
        ...
        for i in range(1, self.cfg.num_iterations + 1):
            ...
            for tid in self.tasks:
                if np.random.rand() < self.cfg.hmcr:
                    ...
                    if np.random.rand() < self.cfg.par:
                        # Pitch adjustment toward best harmony (GHS)
                        best_val = self.best[tid]
                        if best_val not in used:
                            val = best_val
                        elif val in used:
                            remaining = [e for e in self.emps if e not in used]
                            val = np.random.choice(remaining)
                    elif val in used:
                        remaining = [e for e in self.emps if e not in used]
                        val = np.random.choice(remaining)
                else:
            ...
            if self.log_every and i % self.log_every == 0:
                print(f"  GHS iter {i}/{self.cfg.num_iterations} best={self.best_score:.4f}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="GHS assignment by topo levels (one assignee per task per batch).",
    )
    ...
    parser.add_argument(
        "--plot-dir",
        default="projects/ZOOKEEPER/ghs_plots",
...
def main():
    ...
    cfg = HSConfig(hmcr=args.hmcr, par=args.par)
    ...
    for level in sorted(topo[LEVEL_COL].dropna().unique()):
        ...
        for i in range(0, len(tasks_list), batch_size):
            ...
            for tid, emp in best.items():
                ...
                adj_duration = float(info["Duration_Hours"]) * duration_multiplier(gap, cfg)
                all_rows.append(
                    {
                        "Task_ID": tid,
                ...
                if topo_order_col and topo_order_col in info:
                    all_rows[-1]["Topo_Order"] = int(info[topo_order_col])
            ...
            if history:
                try:
                    ...
                    plt.title(f"GHS Objectives - Level {int(level)} Batch {(i // batch_size) + 1}")
                    plt.xlabel("Iteration")
                    plt.ylabel("Score")
                    plt.legend(loc="best", fontsize=8)
                    plt.tight_layout()
                    out_name = f"ghs_objectives_L{int(level)}_B{(i // batch_size) + 1}.png"
                    ...
                    plt.title(f"GHS Scores - Level {int(level)} Batch {(i // batch_size) + 1}")
                    plt.xlabel("Iteration")
                    plt.ylabel("Score")
                    plt.legend(loc="best", fontsize=8)
                    plt.tight_layout()
                    out_name = f"ghs_scores_L{int(level)}_B{(i // batch_size) + 1}.png"
                    plt.savefig(os.path.join(plot_dir, out_name), dpi=150)
                    plt.close()
    ...
    summary = {
        ...
        "avg_quality_score": float(np.mean([s.get("quality_score", 0.0) for s in score_rows])) if score_rows else 0.0,
        "avg_cost_optimization": float(np.mean([s.get("cost_optimization", 0.0) for s in score_rows])) if score_rows else 0.0,
