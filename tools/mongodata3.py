...
PROJECT_KEY = "MJAR"
ISSUES_OUT = r"projects/WODEN/issues.csv"
STATUS_EVENTS_OUT = r"projects/WODEN/status_events.csv"
ISSUE_LINKS_OUT = r"projects/WODEN/issue_links.csv"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Export issues and links from MongoDB.")
    parser.add_argument("--project-key", default=PROJECT_KEY)
    parser.add_argument("--issues-out", default=ISSUES_OUT)
    parser.add_argument("--status-out", default=STATUS_EVENTS_OUT)
    parser.add_argument("--links-out", default=ISSUE_LINKS_OUT)
...
def main():
    ...
    issues_out = os.path.abspath(args.issues_out)
    status_out = os.path.abspath(args.status_out)
    links_out = os.path.abspath(args.links_out)

    for p in [issues_out, status_out, links_out]:
    ...
    with open(issues_out, "w", newline="", encoding="utf-8") as f_issues, \
         open(status_out, "w", newline="", encoding="utf-8") as f_status, \
         open(links_out, "w", newline="", encoding="utf-8") as f_links:
    ...
    print(f"OK. Exported {total_events} status events to {status_out}")
    print(f"OK. Exported {total_links} issue links to {links_out}")
